...
In this study, we evaluate evidence for the association between sleep quality, depression, and quality of life in a diverse sample of adults. These relationships were investigated in the context of low, medium, and high SES as a moderator, as those in lower socioeconomic groups are at higher risk for
depression and sleep problems. Given the importance of sleep quality and mental
health on long-term health outcomes, we hope to enrich the current literature with a better understanding of these complex factors. Findings from this study have implications for psychopathology models and provide a foundation from which causal and longitudinal intervention-focused research can be constructed.
...
## Participants
Participants were recruited through Amazon Mechanical Turk (MTurk). MTurk is a survey tool offered through Amazon.com, where people around the world can complete surveys for monetary compensation. MTurk is commonly used in social science research studies, and it has been found to be representative \textcolor{red}{(CITE)}.

<!-- I think there's a paper by Thomas Leeper that supports the use of MTurk. I have no problem with it and would trust the results but old stogy people might not, so best to provide a citation to back up the claim. -->
...
Participants were also asked to complete a demographics questionnaire, which asked about their socioeconomic status, race/ethnicity, and educational background. 

<!-- You might also want to state the income thresholds for what you defined as low, medium, and high SES -->
...
<!-- If you were to use Qualtrics for an actual study, there is an R package that will allow you pull your data straight from there using the Qualtrics API. See here https://github.com/ropensci/qualtRics -->

## Data analysis
We used `r cite_r("r-references.bib")` for all our analyses. We calculated means and standard deviations for each of the total scale scores, and we separated means by socioeconomic status. We also calculated Pearson's $r$ correlation coefficients between depressive symptoms, quality of life, and sleep quality total scores. We interpreted our correlational results in accordance with Cohen's recommendations \textcolor{red}{(CITE)}, with small correlations ranging from .10 to .29, medium ranging from .30-.49, and large correlations ranging from .50-1.00.

<!-- The cite here should be Cohen, 1988, I believe -->

# Results

<!-- If you have `include = FALSE` you will go no output whatsoever. The code will evaluate, but you won't get any results. I do this to set up global options and load packages, but I wouldn't use it past that. Also, if you have `include = FALSE`, then `message = FALSE` and `error = FALSE` are rather redundant at that point. -->
...
#Import data
list(here()) # were you looking for `list.files` here? As in
# list.files(here())
data <- import(here("Data", "Final_project_simulated_data.csv"))

# I'd probably not call this object data, because there's a base function in R called data that you use to load data from packages.
...
# Rather than changing all the types like this, you can specify it on import using the argument `col_type` argument. I'm not sure if that works with {rio}, actually, but I know it does if you used `readr::read_csv`. From the documentation, you could do something like 
#
# df3 <- read_csv(
...
<!-- You shouldn't need to put `echo = FALSE` as a chunk option because that's the default in {papaja}. -->

<!-- This isn't where the plot is actually rendered, so removed figure options -->
```{r plot-1-set}
# theme had already been set, so I removed that part.
...
```{r plot-1-render, results = "asis", fig.width = 5, fig.height = 8, fig.cap = "Mean depression, sleep quality, and quality of life scores by socioeconomic status"}

# Set the figure caption through `fig.cap` so remove it from the actual figure
plot_grid(pp1, pp2, pp3, ncol = 1)

# Rather than using cowplot, and pasting three plots together, I would have liked to have seen you just use gather with something like
# scorebyses %>%
#   gather(outcome, Mean, -SES) %>%
#   mutate(outcome = gsub("Mean | Score", "", outcome)) %>%
#   ggplot(aes(SES, Mean)) +
#     geom_col(aes(fill = SES, alpha = 0.7, width = 0.5)) +
#     facet_wrap(~outcome, ncol = 1) +
#     guides(fill = FALSE)

# This is not only more succinct, but arguably a better plot because the y-axis is constant and therefore more directly comparable between the facets.

```


## Descriptive Statistics
The mean depression total score for the sample was `r round(mean(data$mdd_total), 2)`, while the mean sleep quality score was `r round(mean(data$sq_total), 2)`, and the mean quality of life score was `r round(mean(data$ql_total), 2)`. Means by socioeconomic status are listed in Table 1 and displayed graphically in Figure 1. 
...
# I'm guessing, since this is using base functions and camelCase, that you found this online? There's nothing wrong with that, but you might want to give subtle credit by linking to where you got the code from in a code comment.

# EDIT: I found it. From Jasmine Dumas! She's great, and actually gave a talk for PDX R Users group a while back.  I would do it something like the following

# The following function was written by Jasmine Dumas, and can be found here: https://github.com/jasdumas/data-exploreR/blob/master/www/flattenCorrMatrix.R
...
```{r plot-2-render, fig.width = 5, fig.height = 8, results = "asis", fig.cap = "Relationships between Depression, Sleep Quality, and Quality of Life Scores in Low SES Individuals"}

plot_grid(sp1, sp2, sp3, ncol = 1)

# I again would have liked to see you use gather here, for the same reasons stated above.
```
...
```{r plot-3, fig.cap = "Low-to-High SES Difference in MDD scores"}
