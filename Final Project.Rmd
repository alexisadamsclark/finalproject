...
ggplot(data_p, aes(x = sq_total, y = mdd_total)) + 
  ...
  labs(title = "The Association between Sleep Quality & Depression in Low SES Individuals",
       x = "Sleep Quality", 
       y = "Depression")
...
ggplot(data_p, aes(x = sq_total, y = ql_total)) + 
  geom_point(position = "jitter", alpha = 0.7) + 
  geom_smooth(method = "lm") +
  labs(title = "The Asssociation between Sleep Quality & Quality of Life in Low SES Individuals",
       x = "Sleep Quality", 
       y = "Quality of Life")
...
ggplot(data_p, aes(x = ql_total, y = mdd_total)) + 
  geom_point(position = "jitter", alpha = 0.7) + 
  geom_smooth(method = "lm") +
  labs(title = "The Association between Quality of Life & Depression in Low SES Individuals",
       x = "Quality of Life", 
       y = "Depression")
```
...
data_t <- data %>%  
                ...
                group_by(en, Measure, Item) %>% 
                summarise(Value_mean = mean(Value)) %>% 
                spread(key = en, value = Value_mean)
kable(data_t)
```
